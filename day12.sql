...
-- 2. DBMS가 OBJECT(객체)로 관리/인식하는 대상을 생성, 수정, 삭제하는 명령어
...
/* ---------------------------------
   ...
   1. PRIMARY KEY : 이 컬럼에 입력되는 값은 중복되지 않고
                    한 행을 식별할 수 있는 값으로 설정해야하며
                    NULL 데이터 입력은 불가한 값이어야 한다.
                    
   2. FOREIGN KEY : 주로 JOIN에 사용되는 제약조건으로 다른 테이블의 PRIMARY KEY로
                    사용되었던 값이 등장해야만 한다.
                    
   3. UNIQUE : 이 컬럼에 입력되는 값은 중복되지 않음을 보장해야 한다.
               NULL 데이터 입력은 가능한다.
               ==> 데이터가 NULL(없거나) 이거나
                   NULL이 아니라면 반드시 유일한 값이어야 한다.
...
-- 3) DROP COLUMN
-- 예 ) 수정한 테이블 member에서 like_number 컬럼 삭제
...
-- 테이블 생성 시, DDL로 정의한 내용은 시스템 카탈로그에 저장됨.
...
-- 복사하여 테이블 생성 시 참이 될 수 있는 조건을 주면
...
-- 2) 컬럼에 데이터가 있을 때 : 데이터가 소실되면 안되므로 변경에 제약이 있음
--                              타입 변경은 같은 타입내에서만 가능
--                              문자 타입 간에 CHAR -> VARCHAR2 변경 가능
...
-- 예) 3월 생일인 멤버만 복사한 MARCH_MEMBER 테이블을 생각해보자.
...
-- 데이터가 없는 컬럼으로 변경 후 VARCHAR(2) 문자컬럼으로 변경
...
-- MAIN_TABLE1, MAIN_TABLE2의 제약조건을 비교
...
/*
MAIN_TABLE1	SYS_C007476	    C
MAIN_TABLE1	SYS_C007477	    P
MAIN_TABLE1	SYS_C007478	    U
...
-- 테이블 삭제 : DROP
-- 두 테이블 사이에 REFERENCES (FOREIGN KEY) 관게가 있을 때의 삭제

-- 예) MAIN_TABLE1의 경우 SUB_TABLE1에 의해 ID컬럼이 참조되고 있는 상태
...
-- SUB_TABLE1이 MAIN_TABLE1의 ID 컬럼을 참조하고 있기때문에 테이블 삭제 시 순서가 필요하다.
...
-- 즉, 제약조건을 모두 삭제히며 테이블을 DROP함
-- 특히 이 결과에서 SUB_TABLE2에 있던 R 제약조건이 같이 사라졌음을 확인

-- SUB_TABLE3을 DROP 한 뒤, user_constraints에서 관련 행이 사라짐을 확인
...
-- SUB_TABLE3을 DROP해도 MAIN_TABLE3