...
-- AND 연사자는 OR 연산자보다 우선순위가 높기 때문에
...
-- 문제 : emp테이블에서 각 사원의 급여를 3으로 나눈 나머지를 조회
...
-- min ~ max 값 사이를 buckets 개수만큼으로 나누고 expr이 출력하는 값이 buckets에 위치하는지 숫자로 알려줌

-- 급여의 범위 : 0 ~ 5000
-- 한 bucket을 1000 단위로 나누면 총 5 개의 bucket이 있음을 알 수 있으므로
-- 각 직원의 급여가 어느 bucket에 위치하는지 구해보자.
-- 그리고 급여 구간 기준으로 오름차순 정렬
