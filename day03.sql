...
-- 인출된 모든 행 : 0
-- >> WHERE 조건 절을 만족하는 행이 emp 테이블에 존재하지 않으므로 조회된 행이 1건도 없음을 의미.
--    오류 결과가 아님에 주의!
...
SELECT e.empno
     ...
     , e.deptno
  FROM emp e
 WHERE e.job = 'CLERK' -- 문자열을 비교할 때는 반드시 비교 문자를 ''로 감싼다
   AND e.deptno = 10 -- 숫자값 비교시에는 따옴표 사용 안 함
;
...
-- 인출된 모든 행 : 0
-- >> Clerk 이라는 값은 데이터에 존재하지 않으므로 CLERK으로 존재하는 데이터와 대소문자가 일치하지 않으면
--    다른 값으로 처리한다.

-- (6) 연산자 : 1. 산술연산자

-- 21) emp 테이블에서 사번, 이름, 급여를 조회하고 
