...
--       comm 컬럼의 값이 (null)인 사람들의 행은 처음부터 비교대상에 들지 않음에 주의해야한다.
--       (null) 값은 비교연산자, 산술연산자로 연산할 수 없는 값이다.
--       단, 정렬에서 null 값은 가장 큰 값으로 취급한다.
...
-- LIKE 연산자 : 유사값을 검색할 때 사용
--                정확한 값을 알지 못할 때 사용
-- LIKE 연산자의 패턴 문자 : 유사 값 검색을 위해 연산자와 함께 사용하는 기호

-- % : 이 기호의 자리에 0자릿수 이상의 모든 문자가 올 수 있음
-- _ : 이 기회의 자리에 1자리의 모든 문자가 올 수 있음

-- 26) 이름이 J로 시작ㅎ는 직원의 사번, 이름 조회
...
SELECT e.empno
     , e.ename
  FROM emp e
 WHERE e.ename LIKE '__M%' -- 패턴 인식 문자(_)를 두 번 사용하여  M 앞자리를 2글자로 제한
;
...
-- : 조회 하려는 값에 패턴인식문자 (%, _ ) 가 들어있는 데이터는 어떻게 조회할 것인가?
SELECT e.empno
     , e.ename
  FROM emp e
...
-- => 위처럼 LIKE 조건을 작성하면 J 뒤에 적어도 1글자가 있는 이름을 가진 직원이 모두 검색됨.
--    원하는 결과가 아님.
--    패턴 인식 문자를 조회하려면 ESCAPE를 사용해야한다.
-- => 패턴 인식 문자를 무효화하려면 ESCAPE를 사용해야한다.
...
-- IS NULL : 비교하려하는 컬럼의 값이 NULL일 때 true(참)
--           비교하려하는 컬럼의 값이 NULL이 아니면 false(거짓)

-- IS NOT NULL : 비교하려하는 컬럼의 값이 NULL이 아니면 true(참)
--               비교하려하는 컬럼의 값이 NULL일 때 false(거짓)

-- NULL 값을 가진 컬럼은 비교연산자 (=, !=, <>) 와 연산이 불가능하므로
-- NULL 값 비교를 위한 연산자가 따로 존재함에 주의!

-- col = NULL == > NULL에 대해서는 동일비교 연산자를 사용 못 함.
-- col != NULL
-- col <> NULL ==> NULL에 대해서는 다름을 비교하는 연산자 (!=. <>)를 사용 못 함
...
SELECT e.empno
     ...
     , e.mgr
  FROM emp e
 WHERE mgr IS NULL -- NULL 데이터는 IS 연산자로 비교해야 함
;
...
-- BETWEEN a AND b :  범위 포함 비교 연산
-- a <= sal <= b : 이러한 범위 연산과 동일한 기능
...
-- EXISTS 연산자 : 어떤 쿼리 (SELECT 구문)을 실행한 결과가 1행 이상일 때 참으로 판단
-- 인출된 모든 행이 0인 경우, 거짓으로 판단
-- 따라서 서브쿼리와 함께 사용됨
...
--      화면에 "급여가 3000이 넘는 직원이 존재함"이라는 메시지를 출력하고 싶다.
...
-- (6) 연산자 : 결합연산자 (||)
--     오라클에만 존재, 문자열 결합(접합)
--     다른 프로그래밍 언어 (Java, C, C++ 등) 에서는 OR 연산자로 사용되므로 혼동해서는 안됌!
...
--4) MINUS : 첫번째 쿼리 실행결과에서 두번째 쿼리 실행결과를 뺀 차집합
SELECT d.deptno
     , d.dname
     , d.loc
  FROM dept d
...
-- 주의 ! : 두 쿼리의 조회 결과의 칼럼 갯수, 데이터 타입의 순서가 일치
-- 1) 오류상황 : 첫쿼리 컬럼 수 : 3, 둘째커리 컬럼 수 : 2
