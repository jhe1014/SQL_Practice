...
--------
TO_CHAR() : 다른 타입에서 문자 타입으로 변환해주는 함수
            숫자, 날짜 => 문자
...
-- 문자화 된 급여는 왼쪽으로 정렬되어 있고 숫자 자체인 급여는 오른쪽으로 정렬되어 있음을 확인한다.
...
-- 2. t_date() : 날짜 패턴에 맞는 문자 값을 날짜 연산이 가능한 날짜 타입으로 변경
...
-- 날짜처럼 생긴 문자와는 날짜 연산이 안됨으로 확인해보자.
...
/*
 만약 default가 설정되어 있지 않고 expr과 일치하는 search도 없는 경우
 DECODE문의 결과는 NULL이 된다.
*/
